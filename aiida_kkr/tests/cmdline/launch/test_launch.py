...
def test_launch_bs(run_cli_process_launch_command, fixture_code):
    ...
    path = os.path.abspath(os.path.join(THISFILE_PATH, '../../files/bd_dump_bs/parent_kkr.tar.zip'))
    remote = generate_remote_data(code.computer, path).store()
    options = ['--kkr', code.uuid, '--parameters', para_node.uuid, '--parent-folder', remote.uuid ]
    run_cli_process_launch_command(launch_bs, options=options)
