...
class kkr_decimation_wc(WorkChain):
    ...
    _wf_default = {'nkz' : 30,   # number of k-points in z-direction for substrate
                   'nprinc': 4,  # number of layer in principle layer
                   'nplayer': 4, # number of principle layers (naez deci: nprinc*nplayer)
                   'dosmode': False, # run DOS calculation
                   'dos_params': {'emin_EF': -5.0, # EMIN-EF in eV
    ...
    _options_default = {'resources': {'tot_num_mpiprocs': 1, 'num_machines': 1}, # resources to allowcate for the job
                        'max_wallclock_seconds' : 60*60,          # walltime after which the job gets killed (gets parsed to KKR)
                        }
    
    _keys2d = ['INTERFACE', '<NLBASIS>', '<RBLEFT>', 'ZPERIODL', '<NRBASIS>', '<RBRIGHT>', 'ZPERIODR', # standard names
               'NLBASIS', 'RBLEFT', 'NRBASIS', 'RBRIGHT' # version of keywords without brackets
              ]
    ...
    def define(cls, spec):
        ...
        spec.input("options", valid_type=Dict, required=False,
                   default=lambda: Dict(dict=cls._wf_default),
                   help="Computer options used in the deicmation step (voronoi and deci-out steps run serially but use the walltime given here).")
        spec.input("remote_data", valid_type=RemoteData, required=True,
                   help="Parent calculation (either previous decimation calculation, then voronoi steps are skipped or slab calculation).")
        ...
        # define outputs        
        spec.output('structure_decimate', valid_type=StructureData, required=True, help="Structure of decimation region.")
        spec.output('structure_substrate', valid_type=StructureData, required=True, help="Structure of substrate lattice continuation.")
        spec.output('out_params_calc_deci_out', valid_type=Dict, help="Output parameter node of deci-out calculation.")
        spec.output('out_params_calc_decimate', valid_type=Dict, help="Output parameter node of decimation calculation.")
        spec.output('out_remote_calc_decimate', valid_type=RemoteData, help="Remote folder of decimation calculation.")
        spec.output('out_retrieved_calc_decimate', valid_type=FolderData, help="Retrieved folder of decimation calculation.")
        ...
        spec.output("dos_data_interpol", valid_type=XyData, required=False,
                   help="interpolated DOS data onto the real axis.")
        ...
        spec.exit_code(300, 'ERROR_INVALID_INPUT_REMOTE_DATA', 'Given remote_data is not correct (needs to be a slab or decimation KKR calculation)')
        spec.exit_code(301, 'ERROR_KKRCODE_NOT_CORRECT', 'The code you provided for kkr does not use the plugin kkr.kkr')
        spec.exit_code(302, 'ERROR_VORONOICODE_NOT_CORRECT', 'The code you provided for voronoi does not use the plugin kkr.voro')
    ...
    def start(self):
        ...
        """
        self.report('INFO: started KKR decimation workflow version {}'
    ...
    def prepare_deci_from_slab(self):
        ...
        out = make_decimation_param_nodes(self.ctx.slab_calc.inputs.parameters,
                                          Float(alat_slab),
    ...
    def _create_structures(self):
        ...
        """
        struc_deci = get_deci_structure(Int(self.ctx.nprinc), Int(self.ctx.nplayer), self.ctx.slab_calc.outputs.remote_folder)
        struc_substrate = get_substrate_structure(Int(self.ctx.nprinc), Int(self.ctx.nplayer), self.ctx.slab_calc.outputs.remote_folder)
...
def get_substrate_structure(nprinc, nplayer, slab_calc):
    ...
    for i in range(nrbasis):
        struc_substrate.append_atom(position=struc.sites[nplayer*nprinc+i].position, symbols=struc.sites[nplayer*nprinc+i].kind_name)
...
def make_decimation_param_nodes(slab_calc_params, slab_alat, struc_deci, nkz, params_overwrite=None):
    ...
    dsubstrate = {k:v for k,v in dsubstrate.get_dict().items() if v is not None} # clean up removing None values from dict
...
def get_noco_angles_deci(init_noco_slab, struc_decimation, struc_substrate):
    ...
    return {'initial_noco_angles_substrate': initial_noco_angles_substrate,
