...
from aiida.orm import Dict, RemoteData, Code, CalcJobNode
...
class kkrimp_BdG_wc(WorkChain):
    ...
    def imp_BdG_calc(self):
        ...
        builder.voronoi = self.inputs.voronoi
        builder.kkr = self.inputs.kkr
        builder.kkrimp = self.input.kkrimp
        builder.startpot = self.ctx.imp_calc.outputs.converged_potential
        builder.remote_data_host = self.input.remote_data_host_BdG
        builder.options = self.input.options
        #builder.metadata.label = 'Fe imp on Nb110'
