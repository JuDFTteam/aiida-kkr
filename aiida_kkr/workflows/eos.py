...
class kkr_eos_wc(WorkChain):
    ...
    def start(self):
        ...
        self.ctx.description = self._wf_description.format(
            self.ctx.structure.get_formula())
        if (
            self.ctx.wf_parameters["settings_kkr_startpot"].get("_label", None)
            is None
        ):
        ...
        if (
            self.ctx.wf_parameters["settings_kkr_startpot"]
            .get("_description", None)
            is None
        ):
            self.ctx.wf_parameters["settings_kkr_startpot"]["_description"] = (
                self.ctx.description
                + " kkr_startpot step for {}"
                .format(self.ctx.structure.get_formula())
            )
        if (
            self.ctx.wf_parameters["settings_kkr_scf"].get("label", None)
            is None
        ):
        ...
        if (
            self.ctx.wf_parameters["settings_kkr_scf"].get("description", None)
            is None
        ):
        ...
        self.ctx.warnings = []
        self.ctx.rms_threshold = (
            self.ctx.wf_parameters["settings_kkr_scf"]
            .get("convergence_criterion", 10**-7)
        )
        ...
        for ii in range(self.ctx.nsteps):
            scale_fac = (
                self.ctx.scale_range[0]
                + ii *
                (
                    self.ctx.scale_range[1] - self.ctx.scale_range[0]
                )/(self.ctx.nsteps-1)
            )
    ...
    def check_voro_out(self):
        ...
        self.report(
            "INFO: updated kkr_parameters including RMTCORE setting (uuid={})"
    ...
    def run_kkr_steps(self):
        ...
        self.report(
            "submit calc for scale fac= {} on {}"
            .format(
                self.ctx.scale_factors[0],
                self.ctx.scaled_structures[0].get_formula()
            )
        )
        future = self.submit(
            kkr_scf_wc, kkr=self.ctx.kkr,
            remote_data=self.ctx.smallest_voro_remote,
            wf_parameters=Dict(dict=wfd),
        ...
        for i in range(len(self.ctx.scale_factors)-1):
            ...
            self.report(
                "submit calc for scale fac= {} on {}"
                .format(scale_fac, scaled_struc.get_formula())
            )
            future = self.submit(
                kkr_scf_wc, structure=scaled_struc, kkr=self.ctx.kkr,
    ...
    def collect_data_and_fit(self):
            ...
            try:
                ...
                if d_result[u"successful"]:
                    ...
                    if rms <= self.ctx.rms_threshold:  # only take those calculations which
                        ...
                        warn = (
                            "rms of calculation with uuid={} not low enough ({} > {})"
                            .format(uuid, rms, self.ctx.rms_threshold)
                        )
        ...
        self.report(
            "{:16} {:8.3f} {:7.3f} {:7.3f} {:7.3f}"
            .format(
                "mean", mean(alldat[:, 0]), mean(alldat[:, 1]),
                mean(alldat[:, 2]), mean(alldat[:, 3])
            )
        )
        self.report(
            "{:16} {:8.3f} {:7.3f} {:7.3f} {:7.3f}"
            .format(
                "std", std(alldat[:, 0]), std(alldat[:, 1]),
                std(alldat[:, 2]), std(alldat[:, 3])
            )
        )
    ...
    def return_results(self):
        ...
        if self.ctx.successful and self.ctx.return_gs_struc:
            ...
            gs_structure.label = (
                "ground_state_structure_{}".format(gs_structure.get_formula())
            )
            gs_structure.description = (
                "Ground state structure of {} after running eos workflow. Uses {} fit."
                .format(gs_structure.get_formula(), self.ctx.fitfunc_gs_out)
            )
