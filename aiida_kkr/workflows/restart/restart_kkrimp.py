...
class KkrimpCalculationBaseWorkChain(CalculationBaseWorkChain):
    ...
    _process_class = KkrimpCalculation

    # list of exit codes we cannot handle automatically
    _exit_codes_nohandler = [
        KkrimpCalculation.exit_codes.ERROR_NO_RETRIEVED_FOLDER,  # pylint: disable=no-member
        KkrimpCalculation.exit_codes.ERROR_PARSING_KKRIMPCALC,  # pylint: disable=no-member
        KkrimpCalculation.exit_codes.ERROR_NO_OUTPUT_FILE,  # pylint: disable=no-member
        KkrimpCalculation.exit_codes.ERROR_OPENING_OUTPUTS,  # pylint: disable=no-member
        KkrimpCalculation.exit_codes.ERROR_CALCULATION_FAILED,  # pylint: disable=no-member
    ]
    # common exit codes for which we know what to do
    _exit_code_memory = KkrimpCalculation.exit_codes.ERROR_NOT_ENOUGH_MEMORY  # pylint: disable=no-member
    _exit_code_timelimit = KkrimpCalculation.exit_codes.ERROR_TIME_LIMIT  # pylint: disable=no-member
    ...
    @process_handler(priority=30, exit_codes=KkrimpCalculation.exit_codes.ERROR_RLOG_TOO_SMALL)  # pylint: disable=no-member
