...
import pytest
from kkr_params import kkrparams
...
class Test_capture_wrong_input():
    ...
    def test_input_consistency_check_fail(self):
        knownError = False
        try: 
            p = kkrparams(LMAX=2, NAEZ=1, BRAVAIS=[[1,0,0],[0,1,0],[0,0,1]], RMAX=7, GMAX=65, NSPIN=2, RBASIS=[0,0,0], ALATBASIS=1)
    ...
    def test_inconsistency_bulk_mode_bravais(self):
        p = kkrparams(LMAX=2, NAEZ=1, BRAVAIS=[[1,0,0],[0,1,0],[0,0,0]], NSPIN=2, RBASIS=[0,0,0], ALATBASIS=1, RMAX=7, GMAX=65)
        knownError = False
        try:
...
class Test_get_info():
    def test_get_mandatory(self):
        ...
        assert set(manlist)==set(['LMAX', 'NAEZ', 'BRAVAIS', 'RMAX', 'GMAX', 'NSPIN', '<RBASIS>', 'ALATBASIS'])
...
class Test_fill_inputfile():
    ...
    def test_fill_inputfile_KKR(self):
        ...
        reffile = ['ALATBASIS= 1.000000\n', 'BRAVAIS\n', '1.000000 0.000000 0.000000\n',
                   ...
                   '<RBASIS>\n', '0.000000 0.000000 0.000000\n', 'NSPIN= 2\n', 'LMAX= 2\n', 
                   'RCLUSTZ= 1.500000\n', 'RMAX= 7.000000\n', 'GMAX= 65.000000\n']
        p = kkrparams(LMAX=2, NAEZ=1, BRAVAIS=[[1,0,0],[0,1,0],[0,0,1]], RMAX=7, GMAX=65, RCLUSTZ=1.5, NSPIN=2, RBASIS=[0,0,0], ALATBASIS=1)
        p.fill_keywords_to_inputfile()
        txt = open('inputcard').readlines()
