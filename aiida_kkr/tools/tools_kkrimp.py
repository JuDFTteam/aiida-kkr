...
class modify_potential():
    ...
    def _check_potstart(self, str1, shape_ver='new'):
        if shape_ver=='new':
    ...
    def shapefun_from_scoef(self, scoefpath, shapefun_path, atom2shapes, shapefun_new):
        ...
        """
        data = open(shapefun_path).readlines()
        
        # read shapefun
        index1=[];index2=[]
        for i in range(len(data)):
          if self._check_potstart(data[i]):
            index1.append(i)
        ...
        if len(index1)<1:
            index1=[];index2=[]
            for i in range(len(data)):
                if self._check_potstart(data[i],shape_ver='old'):
                    index1.append(i)
                if len(index1)>1: index2.append(i-1)
                index2.append(i)
